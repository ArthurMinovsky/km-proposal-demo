...
bash -n scripts/install.sh scripts/ingest.sh scripts/uninstall.sh scripts/status.sh scripts/verify.sh scripts/search.sh

echo "[2/8] required deliverables"
test -f docker-compose.yml
test -f Dockerfile.ingest
test -f tools/ingest.mjs
test -f tools/search.mjs
test -f scripts/search.sh
...
grep -q './scripts/ingest.sh' skill/km-management/SKILL.md
